...
router.patch("/:bookIsbn", async (req, res, next) => {
  try {
    ...
    let query = {$set: {}};
  for (let key in rest) {
    if (book[key] && book[key] !==rest[key]) 
       query.$set[key] = rest[key];}

  const updatedBook = await  BookModel.updateOne({isbn: req.params.bookIsbn}, query)
...
router.delete("/:bookIsbn", async (req, res, next) => {
  try {
    ...
    await BookModel.deleteOne({ isbn: req.params.bookIsbn })
